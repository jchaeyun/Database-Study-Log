...
* **사실(Fact):** `JOIN`은 관계형 데이터베이스의 정체성과 같은 기능이며, 이를 위해 테이블을 정교하게 나누는 과정을 '정규화'라고 한다.
* **구조적 지적:** 입문자는 Workbench의 편리함에 매몰되어 SQL 문법을 소홀히 할 수 있으나, 결국 복잡한 로직은 직접 쿼리를 작성해야 하므로 두 방식을 병행하는 것이 필수적이다.

---

**위 내용을 바탕으로 `author` 테이블을 직접 생성하고 `topic` 테이블과 `JOIN`을 수행해 보겠는가? 필요한 테이블 생성 쿼리(Create Table)와 샘플 데이터를 즉시 제공할 수 있다.**
